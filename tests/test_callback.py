...
def run_playbook_callback(tmpdir, report_type):
    extra_env = {}
    if ansible_version < LooseVersion('2.11'):
...
def drop_incompatible_items(d):
    ...
    for k, v in d.items():
        if k in ['msg', 'start', 'end', 'delta', 'uuid', 'timeout', '_ansible_no_log', 'warn', 'connection',
                 ...
                 'listen', '_ansible_internal_redirect_list', 'exception', 'resolved_action', 'delay', '_resolved_action', 'is_handler', 'changed_when_result',
                 'ignore_errors', 'ignore_unreachable', 'failed']:
            continue
...
def run_callback(tmpdir, report_type, vcrmode):
    ...
    for real_file in tmpdir.listdir(sort=True):
        ...
        if report_type == "foreman":
            ...
            contents = re.sub(r", \\\"changed_when_result\\\": [^,]+", "", contents)
            contents = re.sub(r", \\\"failed\\\": false", "", contents)
        ...
        if vcrmode == "record":
            ...
            with open(fixture, 'r') as f:
                fixture_data = f.read()
                if ansible_version >= LooseVersion('2.22'):
                    fixture_data = fixture_data.replace('admin', '$REDACTED$')
                    fixture_data = fixture_data.replace('ENCRYPTED_VAULT_VALUE_NOT_REPORTED', '$REDACTED$')
                elif ansible_version >= LooseVersion('2.19'):
